...
def calculate_demographic_data(print_data=True):
    ...
    high_earners = df[df['salary'] == '>50K']['native-country'].value_counts(normalize=True)
    low_earners = df[df['salary'] == '<=50K']['native-country'].value_counts(normalize=True)
    ratio = (high_earners / low_earners)
    highest_earning_country = ratio.idxmax()
    highest_earning_country_percentage = round(df[df['native-country'] == highest_earning_country]['salary'].value_counts(normalize=True)['>50K'] * 100,1)
